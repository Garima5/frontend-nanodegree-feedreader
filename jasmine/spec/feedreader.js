...
$(function() {
    ...
    describe('New Feed Selection', function() {
        beforeEach(function(done) {
            loadFeed(0, function() {
                feed1Content = $('.feed').html(); //content before the new feed is loaded
                loadFeed(1, function() { //nesting the loadfeed function
                    feed2Content = $('.feed').html(); //content after the new feed is loaded
                    done();
                });
            });
        });
        it('contains a new feed when a new feed is loaded', function(done) {
            expect(feed1Content).not.toEqual(feed2Content);
            done();
        });
    });
}());
